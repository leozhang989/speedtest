...
// Id doesn't exist
func GetSettingsById(id int64) (v *Settings, err error) {
	o := orm.NewOrm()
	v = &Settings{Id: id}
	if err = o.QueryTable(new(Settings)).Filter("Id", id).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}

// GetSettingsBySettingKey retrieves Settings by SettingKey. Returns error if
// SettingKey doesn't exist
...
func DeleteSettings(id int64) (err error) {
	o := orm.NewOrm()
	v := Settings{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
